...
def find_not_connected_samples(cites_files):
    """
    find_not_connected_samples(ego_nodes) finds non-friend nodes as per complete edge data
    (both training and testing combined) for all the nodes.
    It takes the list of ego nodes as its argument.
    For each node in the ego network, it tries to find as many not connected nodes as the connected nodes for that node,
    in the testing (20%) edges data.
    It saves the found not connected nodes in the directory ../Not_Connected_Samples/
    """

    try:
        os.mkdir('/Users/macbookair/Documents/Project/fItneSS_us/recommendation/datasets/cora/Not_Connected_Samples/')
    except:
        pass
    ...
    for cites in cites_files:
        for i in range(5):
            ...
            file = open('/Users/macbookair/Documents/Project/fItneSS_us/recommendation/datasets/cora/Edges/edges_20_{}_{}.txt'.format(cites, i))
            ...
            file = open('/Users/macbookair/Documents/Project/fItneSS_us/recommendation/datasets/cora/Edges/edges_80_{}_{}.txt'.format(cites, i))
            lines = file.readlines()
            file.close()
            ...
            file = open('/Users/macbookair/Documents/Project/fItneSS_us/recommendation/datasets/cora/Not_Connected_Samples/sample_{}_{}.txt'.format(cites, i), 'w')
            for node in nodes:
